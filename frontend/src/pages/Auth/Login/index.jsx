...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        // STEP 1: REAL BACKEND CALL to send OTP
        if (!otpSent) {
            setIsSubmitting(true);
            setErrorMsg('');
            setSuccessMsg('');
            ...
            try {
                ...
                await api.post('/auth/send-otp', { email });
                setOtpSent(true);
                setSuccessMsg('Secure session verified. Check your email for the OTP.');
            } catch (error) {
                setErrorMsg(error.response?.data?.error || error.response?.data?.message || 'Failed to send OTP.');
            } finally {
                setIsSubmitting(false);
            }
            return;
        ...
        // ✨ Pass the OTP to your AuthContext!
        const result = await login(email, password, otp);
        
        if (!result.success) {
            setErrorMsg(result.error || 'Invalid OTP or login failed. Please try again.');
            setIsSubmitting(false);
        }
    };
    ...
    return (
        <>
            {isSubmitting && <Loader message={otpSent && otp.length === 6 ? "Authenticating session..." : "Verifying credentials..."} />}

            <div className="auth-wrapper">
                <div className="auth-panel">
                    <div className="auth-header">
                        <h2>Welcome Back</h2>
                        <p className="auth-subtitle">Sign in to your secure sandbox</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <button type="submit" className="btn-primary" disabled={isSubmitting}>
                            {!otpSent ? 'Sign In' : 'Verify & Login'}
                        </button>
                    </form>

                    {/* ✨ Resend OTP UI elements */}
                    {otpSent && (
